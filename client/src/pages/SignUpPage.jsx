...
function SignupPage() {
  ...
  const vaidateForm =()=> {
    ...
    if(isValidPassword2){
      ...
      setPasswordError2("Enter Passsword of atlest 8 character and one capital letter must, one small letter must, numbers are allowed and one special char must")
    }
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '65vh' }}>
      ...
      <div className="input-container">
        ...
        {
          passwordError2.length>0? <label style={{color:"var(--error-color)", fontSize: "0.5em"}}>{passwordError2}</label>:<></>
        }
          <div onClick={()=>{setPasswordView2(!passwordView2)}}>
            <img
        ...
        <input type={passwordView2?"text":"password"} placeholder='Enter Password' value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
        </div>
